export default () => {
  const jwtSecret = process.env.API_SERVER_JWT_SECRET || process.env.JWT_SECRET;
  if (!jwtSecret || jwtSecret.length < 32) {
    throw new Error('API_SERVER_JWT_SECRET must be set and at least 32 characters long');
  }
