 ...
 *
 * Captures the canonical schema produced by `synchronize: true` against the
 * full entity graph (40 tables, enums, indices, foreign keys). Schema
 * dumped via `pg_dump --schema-only` after a clean DB rebuild on
 * 2026-05-23 and embedded inline so it ships in dist/ without an extra
 * file copy step.
 *
 * Idempotency: if the `users` table already exists (e.g. created by an
 * earlier `synchronize: true` boot), the migration is a no-op and the
 * runner still records it as applied. On a fresh DB the full SQL runs.
 *
 * Future schema changes go through NEW migration files; this one stays frozen.
 *
 * Generator: scripts/gen-initial-migration.js — re-run to refresh after a
 * baseline schema regen.
 */
...
export class InitialSchema1779525000000 implements MigrationInterface {
  name = 'InitialSchema1779525000000';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (Array.isArray(result) && result.length > 0) {
      // Schema already in place (synchronize bootstrap or previous run).
      return;
    }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    // Destructive — nuke the public schema. Only use on dev DBs.
