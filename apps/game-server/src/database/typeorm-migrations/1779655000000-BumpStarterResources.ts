 ...
 *
 * NOTE: the game-server owns "player_resources" (mineral/gas/energy columns).
 * The legacy "resources" table belongs to the api service (EAV model) and
...
export class BumpStarterResources1779655000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "player_resources" ALTER COLUMN "mineral" SET DEFAULT 500`,
    );
    await queryRunner.query(
      `ALTER TABLE "player_resources" ALTER COLUMN "gas" SET DEFAULT 200`,
    );
    await queryRunner.query(
      `ALTER TABLE "player_resources" ALTER COLUMN "energy" SET DEFAULT 250`,
    );
    ...
    await queryRunner.query(
      `UPDATE "player_resources" SET "mineral" = GREATEST("mineral", 500) WHERE "mineral" < 500`,
    );
    await queryRunner.query(
      `UPDATE "player_resources" SET "gas" = GREATEST("gas", 200) WHERE "gas" < 200`,
    );
    await queryRunner.query(
      `UPDATE "player_resources" SET "energy" = GREATEST("energy", 250) WHERE "energy" < 250`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "player_resources" ALTER COLUMN "mineral" SET DEFAULT 100`,
    );
    await queryRunner.query(
      `ALTER TABLE "player_resources" ALTER COLUMN "gas" SET DEFAULT 50`,
    );
    await queryRunner.query(
      `ALTER TABLE "player_resources" ALTER COLUMN "energy" SET DEFAULT 100`,
    );
