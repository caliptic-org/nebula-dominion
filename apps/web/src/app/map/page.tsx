...
import type { WorldMapHandle, HitTarget, WorldMapProps } from '@/components/game/WorldMap';
...
function ActionPanel({ visible, target, actions, raceColor, onAction, onClose }: ActionPanelProps) {
  const selLabel = () => {
        ...
        <div className="px-4 py-3.5 flex flex-wrap gap-2">
          {actions.map(action => (
            <button
              ...
              className="flex items-center gap-2.5 px-4 py-2.5 rounded-full font-display text-[11px] font-bold tracking-[0.08em] uppercase active:scale-95"
              style={{
            ...
            >
              <span className="text-base leading-none" aria-hidden>{action.icon}</span>
...
export default function WorldMapPage() {
  ...
  const handleAction = useCallback((action: Action) => {
    setFeedback(`${action.icon} ${action.label} komutu verildi`);
  ...
  return (
    <div className="fixed inset-0 overflow-hidden" style={{ background:'#080a10' }}>
      ...
      <div className="absolute inset-0">
        <WorldMap ref={worldMapRef} playerRace={race} onSelect={handleSelect} />
      </div>
      ...
      {feedback && (
        <div
          ...
          style={{ background:'rgba(13,17,23,0.94)', border:`1px solid ${raceColor}45`, borderRadius:'9999px', padding:'0.375rem 1.25rem', backdropFilter:'blur(12px)' }}
        >
          <span className="font-display text-[11px] tracking-wide" style={{ color:raceColor }}>{feedback}</span>
        </div>
      )}
