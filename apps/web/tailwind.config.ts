import type { Config } from 'tailwindcss'

const config: Config = {
  content: [
    './src/pages/**/*.{js,ts,jsx,tsx,mdx}',
    './src/components/**/*.{js,ts,jsx,tsx,mdx}',
    './src/app/**/*.{js,ts,jsx,tsx,mdx}',
  ],
  theme: {
    extend: {
      colors: {
        space: {
          black:   '#080a10',
          panel:   '#0d1117',
          elevated:'#141d2b',
          overlay: 'rgba(8,10,16,0.92)',
        },
        bg: {
          DEFAULT: 'var(--color-bg)',
          surface: 'var(--color-bg-surface)',
          elevated:'var(--color-bg-elevated)',
          overlay: 'var(--color-bg-overlay)',
          card: 'var(--color-bg-card)',
        },
        text: {
          primary:   'var(--color-text-primary)',
          secondary: 'var(--color-text-secondary)',
          muted:     'var(--color-text-muted)',
          inverse:   'var(--color-text-inverse)',
        },
        brand: {
          DEFAULT: 'var(--color-brand)',
          hover:   'var(--color-brand-hover)',
          dim:     'var(--color-brand-dim)',
          glow:    'var(--color-brand-glow)',
        },
        // Race color tokens
        race: {
          zerg: '#44ff44',
          'zerg-dim': 'rgba(68,255,68,0.12)',
          'zerg-glow': 'rgba(68,255,68,0.4)',
          otomat: '#00cfff',
          'otomat-dim': 'rgba(0,207,255,0.12)',
          'otomat-glow': 'rgba(0,207,255,0.4)',
          canavar: '#ff6600',
          'canavar-dim': 'rgba(255,102,0,0.12)',
          'canavar-glow': 'rgba(255,102,0,0.4)',
          insan: '#4a9eff',
          'insan-dim': 'rgba(74,158,255,0.12)',
          'insan-glow': 'rgba(74,158,255,0.4)',
          seytan: '#cc00ff',
          'seytan-dim': 'rgba(204,0,255,0.12)',
          'seytan-glow': 'rgba(204,0,255,0.4)',
        },
        accent: {
          DEFAULT: 'var(--color-accent)',
          hover:   'var(--color-accent-hover)',
          dim:     'var(--color-accent-dim)',
        },
        energy: {
          DEFAULT: 'var(--color-energy)',
          dim:     'var(--color-energy-dim)',
        },
        border: {
          DEFAULT: 'var(--color-border)',
          hover:   'var(--color-border-hover)',
          focus:   'var(--color-border-focus)',
        },
        status: {
          success: 'var(--color-success)',
          danger:  'var(--color-danger)',
          warning: 'var(--color-warning)',
          info:    'var(--color-info)',
        },
        race: {
          zerg:          '#44ff44',
          'zerg-dim':    'rgba(68,255,68,0.12)',
          'zerg-glow':   'rgba(68,255,68,0.4)',
          otomat:        '#00cfff',
          'otomat-dim':  'rgba(0,207,255,0.12)',
          'otomat-glow': 'rgba(0,207,255,0.4)',
          canavar:       '#ff6600',
          'canavar-dim': 'rgba(255,102,0,0.12)',
          'canavar-glow':'rgba(255,102,0,0.4)',
          insan:         '#4a9eff',
          'insan-dim':   'rgba(74,158,255,0.12)',
          'insan-glow':  'rgba(74,158,255,0.4)',
          seytan:        '#cc00ff',
          'seytan-dim':  'rgba(204,0,255,0.12)',
          'seytan-glow': 'rgba(204,0,255,0.4)',
        },
        race: {
          human:     { DEFAULT: 'var(--race-human)',     dim: 'var(--race-human-dim)',     glow: 'var(--race-human-glow)' },
          zerg:      { DEFAULT: 'var(--race-zerg)',      dim: 'var(--race-zerg-dim)',      glow: 'var(--race-zerg-glow)' },
          automaton: { DEFAULT: 'var(--race-automaton)', dim: 'var(--race-automaton-dim)', glow: 'var(--race-automaton-glow)' },
          monster:   { DEFAULT: 'var(--race-monster)',   dim: 'var(--race-monster-dim)',   glow: 'var(--race-monster-glow)' },
          demon:     { DEFAULT: 'var(--race-demon)',     dim: 'var(--race-demon-dim)',     glow: 'var(--race-demon-glow)' },
        },
        mineral: 'var(--race-human)',
        gas:     'var(--color-accent)',
      },
      fontFamily: {
        display: ['var(--font-display)', 'system-ui', 'sans-serif'],
        body:    ['var(--font-body)',    'system-ui', 'sans-serif'],
        mono:    ['var(--font-mono)',    'monospace'],
      },
      backgroundImage: {
        'nebula-gradient': 'var(--gradient-nebula)',
        'hero-gradient':   'var(--gradient-hero)',
        'card-gradient':   'var(--gradient-card)',
        'brand-gradient':  'var(--gradient-brand)',
        'race-gradient':   'var(--gradient-race)',
      },
      boxShadow: {
        'brand-glow':     '0 0 20px var(--color-brand-glow)',
        'accent-glow':    '0 0 20px var(--color-accent-dim)',
        'race-glow':      '0 0 20px var(--color-race-glow)',
        'race-glow-lg':   '0 0 40px var(--color-race-glow), 0 0 80px var(--color-race-dim)',
        'manga-panel':    '0 0 0 2px rgba(255,255,255,0.06), inset 0 1px 0 rgba(255,255,255,0.08)',
        card:             '0 4px 24px rgba(0, 0, 0, 0.5)',
        'card-hover':     '0 8px 48px rgba(0, 0, 0, 0.7)',
      },
      animation: {
        'pulse-slow':     'pulse 3s cubic-bezier(0.4, 0, 0.6, 1) infinite',
        float:            'float 6s ease-in-out infinite',
        'star-twinkle':   'twinkle 4s ease-in-out infinite',
        'glow-pulse':     'glow-pulse 2s ease-in-out infinite',
        'race-glow-pulse':'race-glow-pulse 2.4s ease-in-out infinite',
        'slot-appear':    'slot-appear 400ms cubic-bezier(0.32,0.72,0,1) both',
        shimmer:          'shimmer 2.4s ease-in-out infinite',
      },
      keyframes: {
        float: {
          '0%, 100%': { transform: 'translateY(0px)' },
          '50%':       { transform: 'translateY(-12px)' },
        },
        twinkle: {
          '0%, 100%': { opacity: '0.3' },
          '50%':       { opacity: '1' },
        },
        'glow-pulse': {
          '0%, 100%': { boxShadow: '0 0 10px var(--color-brand-glow)' },
          '50%':       { boxShadow: '0 0 30px var(--color-brand-glow), 0 0 60px var(--color-brand-dim)' },
        },
        'race-glow-pulse': {
          '0%, 100%': { boxShadow: '0 0 8px var(--color-race-glow)' },
          '50%':       { boxShadow: '0 0 24px var(--color-race-glow), 0 0 48px var(--color-race-dim)' },
        },
        'slot-appear': {
          from: { opacity: '0', transform: 'translateY(12px) scale(0.95)' },
          to:   { opacity: '1', transform: 'translateY(0) scale(1)' },
        },
      },
    },
  },
  plugins: [],
}

export default config
